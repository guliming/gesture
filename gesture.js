...
// 						
	...
	var it = {
		...
		'bind': function(){
			...
			//event bind
			evts.addEvent(window, istouch? 'touchstart':'mousedown', it.evts.startAction);
			evts.addEvent(window, istouch?'touchmove':'mousemove', it.evts.moveAction);
			evts.addEvent(window, istouch?'touchend':'mouseup', it.evts.endAction);
			if(istouch){
				evts.addEvent(window, 'touchcancel', it.evts.cancelAction);
			}
			if(hasgesture){
				evts.addEvent(window, 'gesturestart', it.evts.gestureStart);
				evts.addEvent(window, 'gesturechange', it.evts.gestureChange);
				evts.addEvent(window, 'gestureend', it.evts.gestureEnd);
			}
		},
		'evts': {
